...
class Trainer:
    ...
    def _step(
        ...
        # Multi-video batch: model returns list of (per_video_outputs, per_video_targets).
        # Compute loss per video and average.
        if isinstance(outputs, list) and len(outputs) > 0 and isinstance(outputs[0], tuple):
            key = batch.dict_key
            batch_size = len(batch.img_batch)
            all_losses = []
            for vid_outputs, vid_targets in outputs:
                all_losses.append(self.loss[key](vid_outputs, vid_targets))
            # Average loss dicts across videos
            loss = {}
            for k in all_losses[0]:
                loss[k] = sum(l[k] for l in all_losses) / len(all_losses)
        else:
            # Single-video batch: original path
            targets = batch.masks
            batch_size = len(batch.img_batch)
            key = batch.dict_key
